...
RUN_ID="${RUN_ID:-ios-release-background-$(date +%Y%m%d%H%M%S)}"
RESULT_SOURCE="Documents/ai_chat/release_background_smoke"
READY_MARKER="SIMICHAT_RELEASE_BACKGROUND_READY"
NEEDS_NORMAL_RESTORE=0

source scripts/lib/release_pubspec_hook.sh

if ! command -v xcrun >/dev/null 2>&1; then
  echo "xcrun is required for iOS release background smoke" >&2
  exit 1
fi
...
resolve_core_device() {
  local requested="$1"
  local device_info
  device_info="$(mktemp /tmp/simichat-ios-background-device.XXXXXX)"
...
install_release_app() {
  local device="$1"
  local install_json
  install_json="$(mktemp /tmp/simichat-ios-background-install.XXXXXX)"
...
launch_release_app() {
  local device="$1"
  local launch_json
  launch_json="$(mktemp /tmp/simichat-ios-background-launch.XXXXXX)"
...
assert_device_unlocked_for_launch() {
  ...
  preflight_json="$(mktemp /tmp/simichat-ios-background-unlock-preflight.XXXXXX)"
  ...
  if grep -qi 'Locked' "$preflight_json" 2>/dev/null; then
    echo "Refusing to install iOS background smoke build while device is locked" >&2
    cat "$preflight_json" >&2 || true
    rm -f "$preflight_json"
    exit 2
  fi

  echo "Refusing to install iOS background smoke build because launch preflight did not prove the device is unlocked" >&2
  cat "$preflight_json" >&2 || true
  rm -f "$preflight_json"
  exit 2
}
...
find_result_file() {
  local destination="$1"
  find "$destination" -name 'ios-release-background-smoke.json' -type f | head -1
}
...
restore_normal_release() {
  ...
  echo "Restoring normal iOS release build without background smoke dart-defines..."
...
RESULT_DIR="$(mktemp -d /tmp/simichat-ios-background-result.XXXXXX)"
cleanup() {
  local status=$?
...
echo "Building iOS release background smoke runId=$RUN_ID"
"$FLUTTER_BIN" --no-version-check build ios --release \
  --dart-define=SIMICHAT_RELEASE_BACKGROUND_SMOKE=true \
  --dart-define=SIMICHAT_RELEASE_BACKGROUND_SMOKE_RUN_ID="$RUN_ID"
...
if [[ "$ready_seen" != "1" ]]; then
  echo "Timed out waiting for iOS release background ready marker" >&2
  restore_normal_release "$DEVICE_ID"
  exit 1
fi

echo "Suspending iOS Runner pid $LAUNCHED_PID"
xcrun devicectl device process suspend \
  --device "$DEVICE_ID" \
  --pid "$LAUNCHED_PID" \
  --timeout 10 >/dev/null
sleep 3
echo "Resuming iOS Runner pid $LAUNCHED_PID"
  ...
  if copy_smoke_result "$DEVICE_ID" "$RESULT_DIR"; then
    ...
    if [[ -n "$result_file" ]]; then
      ...
      if [[ "$status_output" == passed* ]]; then
        echo "iOS release background smoke passed:"
        echo "$status_output"
        smoke_status=0
        break
      fi
...
if [[ "$smoke_status" == "2" ]]; then
  echo "Timed out waiting for iOS release background smoke pass" >&2
  if [[ -n "$result_file" ]]; then
