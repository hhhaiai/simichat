...
RUN_ID="${RUN_ID:-ios-release-deep-link-$(date +%Y%m%d%H%M%S)}"
RESULT_SOURCE="Documents/ai_chat/release_deep_link_smoke"
RESULT_FILE_NAME="ios-release-deep-link-smoke.json"
SETTINGS_READY_MARKER="SIMICHAT_RELEASE_DEEP_LINK_SETTINGS_READY"
SETTINGS_URL="ai-chat://settings"
SESSION_URL="ai-chat://session/ios-release-deep-link-target-session"
...
if ! command -v xcrun >/dev/null 2>&1; then
  echo "xcrun is required for iOS release deep link smoke" >&2
  exit 1
fi
...
resolve_core_device() {
  local requested="$1"
  local device_info
  device_info="$(mktemp /tmp/simichat-ios-deeplink-device.XXXXXX)"
...
install_release_app() {
  local device="$1"
  local install_json
  install_json="$(mktemp /tmp/simichat-ios-deeplink-install.XXXXXX)"
...
launch_release_app() {
  ...
  local payload_url="${2:-}"
  local launch_json
  launch_json="$(mktemp /tmp/simichat-ios-deeplink-launch.XXXXXX)"
  local args=(
...
assert_device_unlocked_for_launch() {
  ...
  preflight_json="$(mktemp /tmp/simichat-ios-deeplink-unlock-preflight.XXXXXX)"
  echo "Checking iOS device unlock state before installing deep link smoke build..."
  ...
  if grep -qi 'Locked' "$preflight_json" 2>/dev/null; then
    echo "Refusing to install iOS deep link smoke build while device is locked" >&2
    cat "$preflight_json" >&2 || true
    rm -f "$preflight_json"
    exit 2
  fi

  echo "Refusing to install iOS deep link smoke build because launch preflight did not prove the device is unlocked" >&2
  cat "$preflight_json" >&2 || true
  rm -f "$preflight_json"
  exit 2
}
...
result_status() {
  local file="$1"
  python3 - "$file" "$RUN_ID" "$SETTINGS_READY_MARKER" <<'PY'
import json
import sys

path, run_id, settings_marker = sys.argv[1:4]
data = json.load(open(path))
if data.get("runId") != run_id:
    print("mismatch")
    sys.exit(2)
status = data.get("status")
if status == "settings_ready" and data.get("marker") == settings_marker:
    print("settings_ready")
    sys.exit(0)
if status == "passed":
    print("passed")
...
restore_normal_release() {
  ...
  echo "Restoring normal iOS release build without deep link smoke dart-defines..."
...
RESULT_DIR="$(mktemp -d /tmp/simichat-ios-deeplink-result.XXXXXX)"
cleanup() {
  local status=$?
...
echo "Building iOS release deep link smoke runId=$RUN_ID"
"$FLUTTER_BIN" --no-version-check build ios --release \
  --dart-define=SIMICHAT_RELEASE_DEEP_LINK_SMOKE=true \
  --dart-define=SIMICHAT_RELEASE_DEEP_LINK_SMOKE_RUN_ID="$RUN_ID"
...
launch_release_app "$DEVICE_ID" "$SETTINGS_URL"
...
settings_ready_seen=0
result_file=""
for _ in $(seq 1 45); do
  if copy_smoke_result "$DEVICE_ID" "$RESULT_DIR"; then
    result_file="$(find_result_file "$RESULT_DIR")"
    if [[ -n "$result_file" ]]; then
      status_output="$(result_status "$result_file" || true)"
      if [[ "$status_output" == settings_ready* ]]; then
        settings_ready_seen=1
        break
      fi
      if [[ "$status_output" == passed* ]]; then
        echo "iOS release deep link smoke unexpectedly passed before session URL:"
        echo "$status_output"
        settings_ready_seen=1
        break
      fi
...
if [[ "$settings_ready_seen" != "1" ]]; then
  echo "Timed out waiting for iOS release deep link settings marker" >&2
  restore_normal_release "$DEVICE_ID"
  exit 1
fi

echo "Opening iOS release deep link: $SESSION_URL"
launch_release_app "$DEVICE_ID" "$SESSION_URL"
...
for _ in $(seq 1 45); do
  if copy_smoke_result "$DEVICE_ID" "$RESULT_DIR"; then
    ...
    if [[ -n "$result_file" ]]; then
      ...
      set -e
      if [[ "$status_output" == passed* ]]; then
        echo "iOS release deep link smoke passed:"
        echo "$status_output"
        smoke_status=0
        break
      fi
...
if [[ "$smoke_status" == "2" ]]; then
  echo "Timed out waiting for iOS release deep link smoke pass" >&2
  if [[ -n "$result_file" ]]; then
