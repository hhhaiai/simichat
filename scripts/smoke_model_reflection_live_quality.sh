...
SIMICHAT_LIVE_MODEL_RUNS="${SIMICHAT_LIVE_MODEL_RUNS:-3}"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for the remote model quality gate" >&2
  exit 1
fi
...
if [[ ! "$SIMICHAT_LIVE_MODEL_RUNS" =~ ^[1-5]$ ]]; then
  echo "SIMICHAT_LIVE_MODEL_RUNS must be between 1 and 5" >&2
  exit 2
fi

export MODEL_CONFIG_FILE
export SIMICHAT_LIVE_MODEL_RUNS

flutter --no-version-check test --no-pub --no-test-assets -r expanded \
  tool/model_reflection_live_quality_test.dart \
  --plain-name 'remote OpenAI-compatible model produces safe multi-day long-conversation reflections'
