...
TEST_TARGET="integration_test/mobile_audio_focus_loss_smoke_test.dart"

if [[ "$DEVICE_ID" == *-* ]]; then
  cat >&2 <<'EOF'
iOS devices must be validated with release-only flows for this project.
This Android audio-focus smoke uses the debug integration runner and is only for adb devices.
EOF
  exit 2
fi
