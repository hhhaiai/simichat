...
void main() {
  ...
  testWidgets('external Android app audio focus stops native playback', (
    tester,
  ) async {
    final tempRoot = await getTemporaryDirectory();
    final audioDir = Directory('${tempRoot.path}/external_audio_focus_smoke')
      ..createSync(recursive: true);
    final audioFile = File('${audioDir.path}/simichat-external-focus.wav');
    await audioFile.writeAsBytes(
      _buildSilentWav(durationMs: 9000),
      flush: true,
    );
    ...
    // The host smoke script waits for this marker, then launches an external
    // helper APK that requests AUDIOFOCUS_GAIN_TRANSIENT from another package.
    // ignore: avoid_print
    print('SIMICHAT_EXTERNAL_AUDIO_FOCUS_READY');
...
List<int> _buildSilentWav({int sampleRate = 44100, int durationMs = 1800}) {
  ...
  // playback/focus events without producing an audible tone.
  return bytes;
}
