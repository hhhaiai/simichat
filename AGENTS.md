...
| 2026-07-06 | 真机集成发送 smoke 入口 | 新增 `integration_test/mobile_real_send_smoke_test.dart` 和 `scripts/smoke_device_integration_send.sh`，测试在设备内启动本地 OpenAI 兼容 SSE mock，使用内存 SQLite seed 渠道 / 模型 / 会话，并通过真实 Widget 输入与发送验证 UI → `sendMessage` → SSE → assistant 落库 → UI 展示闭环；脚本会自动临时追加并恢复 `sqlite3.source=system` hook；Pixel 8 已通过；iPhone13 构建可达但 Xcode debug session `CONFIGURATION_BUILD_DIR` 超时，普通 debug 启动同样复现，iOS 发送链路仍待继续补证 | 已完成 Pixel 8 / iOS 待补 |
...
| 优先级 | 待办 | 说明 |
| --- | --- | --- |
| P0 | 移动端真机主链路冒烟 | 已完成 Pixel 8 覆盖安装与启动、专用 72 条长会话显示、手动 Dreaming / Reflection 弹窗和短期提示预览、mock OpenAI 真实发送 / SSE 回复 / 重试 / 停止慢流 / 历史搜索 / 模型切换与切换后发送验证，并新增可重复真机集成发送 smoke 脚本且 Pixel 8 通过；iPhone13 release 覆盖安装与进程可见性已完成，但同一集成发送测试和普通 debug 启动均卡在 Xcode debug session `CONFIGURATION_BUILD_DIR` 超时；仍需 iOS 发送链路、语音、网络切换、后台恢复等交互验证 |
...
| `docs/mobile-device-integration-send-smoke-2026-07-06.md` | 真机集成发送 smoke 入口 / 脚本、Pixel 8 通过证据和 iPhone13 Xcode debug session 卡点记录 |
