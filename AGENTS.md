...
| 方向 | 当前状态 | 下一步重点 |
| --- | --- | --- |
| 移动端 MVP | 基础聊天、会话列表、模型切换、本地 SQLite、Markdown 原始档案、移动端应用名 `SimiAIChat`、包名 `top.simitalk.aichat`、主题与 90%–120% 全局字体、移动端自动化 smoke 已落地，Pixel 8 真实发送 / 重试 / 停止 / 历史搜索 / 模型切换已补证，真机集成发送 smoke 入口已新增并在 Pixel 8 通过 | 真机主链路：iOS 发送链路、设置页、长会话、语音、网络切换、后台恢复 |
...
| 2026-07-06 | 真机集成发送 smoke 入口 | 新增 `integration_test/mobile_real_send_smoke_test.dart`，测试在设备内启动本地 OpenAI 兼容 SSE mock，使用内存 SQLite seed 渠道 / 模型 / 会话，并通过真实 Widget 输入与发送验证 UI → `sendMessage` → SSE → assistant 落库 → UI 展示闭环；Pixel 8 已通过，iPhone13 构建 / 安装可达但 Dart VM Service 未发现，iOS 发送链路仍待继续补证 | 已完成 Pixel 8 / iOS 待补 |
...
| 2026-07-06 | 真机集成发送 smoke：`integration_test/mobile_real_send_smoke_test.dart`、设备内 `dart:io` OpenAI SSE mock、内存 SQLite seed、Pixel 8 真机运行、iPhone13 尝试 | Pixel 8 命令内临时 `sqlite3.source=system` 后运行 `flutter --no-version-check test integration_test/mobile_real_send_smoke_test.dart -d 37101FDJH0077P --no-pub -r expanded` 通过，验证 `Integration Mock OpenAI / integration-mock-model`、用户输入 `device integration send`、mock 收到 `model=integration-mock-model` / `stream=true`、assistant `DEVICE integration reply 20260706` 落库并在 UI 展示；正式 `pubspec.yaml` 不保留临时 hook；iPhone13 同一测试 Xcode build / install 可达，但未发现 Dart VM Service，输出 `No tests ran`，iOS 发送仍待补 |
...
| 优先级 | 待办 | 说明 |
| --- | --- | --- |
| P0 | 移动端真机主链路冒烟 | 已完成 Pixel 8 覆盖安装与启动、专用 72 条长会话显示、手动 Dreaming / Reflection 弹窗和短期提示预览、mock OpenAI 真实发送 / SSE 回复 / 重试 / 停止慢流 / 历史搜索 / 模型切换与切换后发送验证，并新增可重复真机集成发送 smoke 且 Pixel 8 通过；iPhone13 release 覆盖安装与进程可见性已完成，但同一集成发送测试仍卡在 Dart VM Service 未发现；仍需 iOS 发送链路、语音、网络切换、后台恢复等交互验证 |
...
| `docs/mobile-device-integration-send-smoke-2026-07-06.md` | 真机集成发送 smoke 入口、Pixel 8 通过证据和 iPhone13 Dart VM Service 卡点记录 |
